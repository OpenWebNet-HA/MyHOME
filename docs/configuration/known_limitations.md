...
| **The Home Assistant core test environment pins an older core** (2024.3) for the local suite; behaviour on the current core is covered by the `ha-upstream-compat` workflow, not by the local run. | Test harness compatibility. | — |
