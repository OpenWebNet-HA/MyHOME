...
| `copied_from` | entity id | No | The cover the times were taken from; `calibration_source` becomes `copied` and `copied_from` is exposed as an attribute (the card's *Apply these times to other covers* uses this). | `cover.living_room_west` |
