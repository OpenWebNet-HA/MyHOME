...
- **MyHOME Sidepanel (preview)**: An administrator panel available through **Configure → MyHOME panel**, with an optional sidebar shortcut, gateway status, searchable device/entity lists, native Home Assistant name/area editing, and a bus monitor scoped to the selected gateway. See the [sidepanel guide](docs/sidepanel.md) for scope, installation, and testing.
