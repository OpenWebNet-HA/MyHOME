...
test("legacy card adapter uses the same view and keeps both names and Lovelace configuration", async () => {
  ...
  const core = new URL("../../custom_components/myhome/frontend/panel/panel-bus-monitor-view.js?v=0.29.0", import.meta.url).href;
  const source = await readFile(new URL("../../custom_components/myhome/frontend/myhome-bus-card.js", import.meta.url), "utf8");
  assert.match(source, /from "\/myhome_static\/panel\/panel-bus-monitor-view.js\?v=0.29.0"/);
  window.customCards = [{ type: "myhome-bus-card" }, { type: "myhome-openwebnet-bus-monitor" }, { type: "unrelated-card" }];
  await import(`data:text/javascript,${encodeURIComponent(source.replace('/myhome_static/panel/panel-bus-monitor-view.js?v=0.29.0', core))}`);
