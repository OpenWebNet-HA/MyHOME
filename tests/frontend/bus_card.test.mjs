...
test("live stopwatch: start down sends close_cover and stop & save calls set_cover_travel_time", async () => {
  ...
  // Advance fake elapsed time
  card._stopwatch.elapsed = 18.42;
...
test("dynamic mode detection: legacy gateway MH200/MH200N defaults to manual mode with single [Time] button", async () => {
  const card = withCovers(create({ model: "MH200" }), {
    "cover.a": coverState("cover.a", { friendly_name: "Bedroom", travel_time_down: 25, travel_time_up: 25 }),
  });
  await card._toggleCovers();
  assert.equal(card._detectCoversMode(), "manual");
  assert.match(card._coversModeReason(), /MH200/);
  assert.match(el("covers-mode-hint").textContent, /MH200/);
  ...
  // Bottom [Calibrate all covers] button is hidden in manual mode
  assert.equal(el("btn-calibrate-all").style.display, "none");
});

test("dynamic mode detection: 60s actuator safety cutoff signature defaults to manual mode", async () => {
  // Generic modern gateway but cover has ~61.5s (actuator safety cutoff signature)
  ...
  await card._toggleCovers();
  assert.equal(card._detectCoversMode(), "manual");
  assert.match(card._coversModeReason(), /60s actuator safety cutoff/);
  assert.equal(el("btn-calibrate-all").style.display, "none");
...
test("on unsupported gateway MH200: auto mode button is disabled and cannot be selected", async () => {
  const card = withCovers(create({ model: "MH200" }), {
    "cover.a": coverState("cover.a", { friendly_name: "Bedroom", travel_time_down: 25 }),
  });
  await card._toggleCovers();
  assert.equal(card._detectCoversMode(), "manual");
