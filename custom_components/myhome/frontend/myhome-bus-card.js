...
class MyHomeBusCard extends HTMLElement {
      ...
      <ha-card>
        ...
        <div id="covers-panel" class="covers-panel">
          ...
          <div id="covers-callout" class="callout-box">
            ...
            <p>Standard BTicino actuators (F411/U2, F401, modular relay units) have no current-sensing circuitry. Unless configured with physical <code>T</code> configurators, the actuator relay stays energized for a factory-default <strong>60-second safety cutoff</strong>, regardless of curtain height or motor limits. On such actuators a run ends at ~61 s; the backend refuses to store it and asks for the stopwatch instead.</p>
          ...
          <p id="covers-footer-note" class="warn">Measured values are the actuator's run times; they equal the physical travel when the installer calibrated the actuator. A run that ends at the actuator's factory 60 s limit is refused, not stored; a run-time limit the installer set longer than the travel is stored as is. If a shutter visibly stops before the timer ends, set <code>travel_time</code> manually using the stopwatch or direct inputs above.</p>
        </div>
  ...
  // Whether bus auto-calibration can work is a property of the actuator, not
  // of the gateway, and the backend refuses a run that ends at the actuator's
  // 60 s limit whoever calls the service. The card only pre-selects the
  // stopwatch when a cover already carries that signature.
  ...
  _coversModeReason() {
    ...
    if (has60sCutoff) {
      return "🚫 Bus auto-calibration not supported: a cover carries the 60 s actuator run-time limit (~61 s), not a measured travel";
    }
    if (this._userCoversMode) {
  ...
  async _handleExportCalibrationTrace() {
    ...
    if (this._hass && this._hass.callWS) {
      try {
        // Same gateway selection as every other request (mac from the card
        // config, else the primary gateway): the backend returns that gateway's
        // frames only, never another gateway's runs. The export names the
        // gateway by its prefix only, like every other export (no full MAC).
        const res = await this._hass.callWS(this._wsPayload("myhome/cover/calibration_trace"));
  ...
  _renderCoversList() {
    ...
    if (autoBtn) {
      ...
      if (!supported) {
        ...
        autoBtn.title = "Bus auto-calibration is not supported by these actuators (60 s run-time limit instead of an end-stop stop).";
      } else {
    ...
    if (footerNote) {
      if (mode === "manual") {
        ...
        footerNote.textContent = "Measured values are the actuator's run times; they equal the physical travel when the installer calibrated the actuator. A run that ends at the actuator's factory 60 s limit is refused, not stored; a run-time limit the installer set longer than the travel is stored as is. If a shutter visibly stops before the timer ends, set travel_time manually using the stopwatch or direct inputs.";
      }
    }
  ...
  async _startStopwatch(entityId, direction) {
    ...
    // The click is not the motor start: the frame waits in the send queue and the
    // motor starts ~0.55 s after the write (#302). The clock runs from the click
    // until the entity reports its motion anchor (motion_started_at, the write /
    // motor-start echo), then re-anchors on that. The saved value is the run as
    // the backend measured it (last_run_seconds), never this clock.
    const startTime = Date.now();
    const runBefore = this._lastRunMarker(entityId);
    this._stopwatch = {
      entity_id: entityId,
      direction: direction,
      startTime: startTime,
      ...
      timerId: setInterval(() => {
        const sw = this._stopwatch;
        if (!sw || sw.entity_id !== entityId) return;
        if (!sw.anchored) {
          const anchor = this._motionAnchor(entityId);
          if (anchor != null) { sw.startTime = anchor; sw.anchored = true; }
        }
        sw.elapsed = (Date.now() - sw.startTime) / 1000;
        const clockEl = this.shadowRoot && this.shadowRoot.getElementById(`stopwatch-clock-${entityId}`);
        if (clockEl) clockEl.textContent = sw.elapsed.toFixed(1) + "s";
      }, 100),
    };
    this._renderCoversList();
  }

  _coverAttrs(entityId) {
    const st = this._hass && this._hass.states && this._hass.states[entityId];
    return (st && st.attributes) || {};
  }

  /** Epoch ms of the backend's motion anchor for the current run, or null before it is known. */
  _motionAnchor(entityId) {
    const iso = this._coverAttrs(entityId).motion_started_at;
    if (!iso) return null;
    const ms = Date.parse(iso);
    return Number.isFinite(ms) ? ms : null;
  }

  _lastRunMarker(entityId) {
    return this._coverAttrs(entityId).last_run_ended_at || null;
  }

  /** Wait for the backend to report the run that just ended; null if it never does. */
  async _awaitMeasuredRun(entityId, runBefore, timeoutMs) {
    const deadline = Date.now() + timeoutMs;
    for (;;) {
      const a = this._coverAttrs(entityId);
      if (a.last_run_ended_at && a.last_run_ended_at !== runBefore && Number.isFinite(Number(a.last_run_seconds))) {
        return Number(a.last_run_seconds);
      }
  ...
  async _stopAndSaveStopwatch(entityId, { timeoutMs = 8000 } = {}) {
    ...
    // The stop frame leaves the queue after the click; the backend measures the
    // run to the stop's write time (the same anchor #318 uses for the position).
    const measured = await this._awaitMeasuredRun(entityId, this._stopwatch.runBefore, timeoutMs);
    const source = measured != null ? "measured on the bus" : "stopwatch only - the bus did not confirm the run";
    const elapsed = Math.max(1, Math.round((measured != null ? measured : this._stopwatch.elapsed) * 10) / 10);
    ...
    try {
      ...
      this._showBanner(
        ...
        `<span><strong>⏱️ Measured &amp; saved:</strong> ${direction === "down" ? "Down" : "Up"} = ${elapsed.toFixed(1)}s (${source})</span>`,
        6000
      );
    } catch (err) {
      this._showBanner(
