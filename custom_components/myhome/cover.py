...
class MyHOMECover(MyHOMEEntity, CoverEntity):
    ...
    async def _await_motion_anchor(self, written) -> float:
        ...
        if isinstance(written, asyncio.Future):
            # asyncio.wait neither raises for a cancelled delivery future nor
            # swallows a cancellation of *this* task (which must propagate).
            await asyncio.wait({written}, timeout=WRITE_TIMEOUT)
    ...
    async def async_calibrate(self) -> dict:
        ...
        """
        if self._advanced:
            raise HomeAssistantError(f"{self._attr_name} reports its position; calibration is not needed")
    ...
    async def async_set_cover_position(self, **kwargs):
        ...
        if self._calibrating:
            raise HomeAssistantError(f"{self.entity_id} is being calibrated; try again when it has finished")
